...
class GateClient:
    def __init__(self,
                credentials: Optional[GateCredentials] = None,
    ...
    def _generate_signature(self, params: Dict[str, str]) -> str:
        # TODO: Edit generating signature for acces account 
        if not self.credentials:
            raise GateAPIException("API credentials required for authenticated requests")
    ...
    async def _make_request(self, method: str, endpoint: str, params: Optional[Dict] = None) -> Dict:
        async with self._request_semaphore:
            ...
            if self.credentials:
                ...
                headers.update({
                    "X-BM-KEY": self.credentials.api_key,
                    "X-BM-SIGN": signature,
            ...
            try:
                ...
                async with self._session.request(method, url, headers=headers, json=params) as response:
                    ...
                    if response.status != 200 :
                        raise GateAPIException(f"API Error: {response.status}")
                    
                    data = json.loads(text_response)
                    return data
            except aiohttp.ClientError as e:
                raise GateAPIException(f"Network error: {str(e)}")
            except Exception as e:
                raise GateAPIException(f"Unexpected error: {str(e)}")
        
        
    async def get_trading_pairs(self) -> List[TradingPair]:
        response = await self._make_request('GET', '/spot/currency_pairs')
        
        trading_pairs = []
        for pair in response:
            if pair["trade_status"].lower() == "tradable": 
    ...
    async def get_orderbook(self, symbol: str, limit: int = 5) -> OrderBook:
        if limit <= 0 or limit > 50 :
            raise GateAPIException("Limit must be > 0 and <= 50")
        ...
        response = await self._make_request("GET", "/spot/order_book", params)
