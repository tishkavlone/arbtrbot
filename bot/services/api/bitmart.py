...
class BitMartAPIException(Exception):
    pass
...
class BitMartClient:
    ...
    def _generate_signature(self, params: Dict[str, str]) -> str:
        if not self.credentials:
            raise BitMartAPIException("API credentials required for authenticated requests")
            ...
            try:
                ...
                async with self._session.request(method, url, headers=headers, json=params) as response:
                    ...
                    if response.status != 200 :
                        raise BitMartAPIException(f"API Error: {response.status}")
                    
                    data = json.loads(text_response)
                    return data
            except aiohttp.ClientError as e:
                raise BitMartAPIException(f"Network error: {str(e)}")
            except Exception as e:
                raise BitMartAPIException(f"Unexpected error: {str(e)}")
    ...
    async def get_orderbook(self, symbol: str, limit: int = 5) -> OrderBook:
        if limit <= 0 or limit > 50 :
            raise BitMartAPIException("Limit must be > 0 and <= 50")
        ...
        response = await self._make_request("GET", "/spot/quotation/v3/books", params)
        data = response["data"]
